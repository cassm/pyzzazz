...
bool subscriberLoop(std::function<void(void)> resetBackoffCounter)
{
  ...
  redis.hset("pyzzazz:mac2ip", MAC_ADDR.c_str(), WiFi.localIP().toString().c_str());
...
/*
  ...
  while (true) {
    uint64_t millis_now = millis();
    if (millis_now - last_cmd_check > cmd_check_interval) {
        last_cmd_check = millis_now;
        
        String msg = redis.lpop(cmd_channel.c_str());
        if (msg && msg != "") {
            resetBackoffCounter();
...
void updateColours(char* colourString, int bufflen) {
    ...
    for (int led = 0; led < min(maxLeds, numLeds); led++) {
      ...
      for (int ch = 0; ch < 3; ch++) {
        ledVal[ch] = uint8_t(colourString[led*3+ch]);
